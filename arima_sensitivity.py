...
def auto_select_pq(series, p=5, q=5):
    best_aic = np.inf
    best_order = None
    best_model = None
...
def main(country, save,p,q):
    ...
    # 自动选择 p, q 值
    best_model, best_order, aic = auto_select_pq(diff_series, p, q)
    ...
    if save:
        fig1.savefig(SAVE_FOLDER + f"{country}_{BEGIN_YEAR}_{N_FORECAST}_{p=}_{q=}_acf-pacf.png")
        fig2.savefig(SAVE_FOLDER + f"{country}_{BEGIN_YEAR}_{N_FORECAST}_{p=}_{q=}_forecast.png")
        fig3.savefig(
            SAVE_FOLDER + f"{country}_{BEGIN_YEAR}_{N_FORECAST}_{p=}_{q=}_resid-acf-pacf.png"
        )
        fig4.savefig(SAVE_FOLDER + f"{country}_{BEGIN_YEAR}_{N_FORECAST}_{p=}_{q=}_qqplot.png")
...
if __name__ == "__main__":
    ...
    with open("arima_results.csv", "w") as file:
        file.write(
            "NOC,forecast,conf_int_90_lower,conf_int_90_upper,conf_int_95_lower,conf_int_95_upper,conf_int_99_lower,conf_int_99_upper,normal,p_value,aic,p,q\n"
        )
    ...
    for i, NOC in enumerate(countries):
        with open("arima_results.csv", "a") as file:
            ...
            file.write(
                f"{NOC},{forecast[-1]},{conf_int_90},{conf_int_95},{conf_int_99},{normal},{p_value},{aic},{p},{q}\n"
            )
